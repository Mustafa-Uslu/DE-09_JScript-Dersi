...
const BesIsim=userTodos.forEach((a)=>console.log(a.title, a.completed));
...
userTodos
  ...
  .forEach((a) => console.log(` ${a.completed} --- ${a.title} `)); //12 true veri
  ...
  userTodos.push({userId:6, title:"kopegi gezdir", id:101, completed:false});
  console.log(userTodos);


  console.clear();  //ekrani temizledik.
...
//const{price,title}=product   //yerine üst satira { ekledik}
console.log(price, title)
);

//? extra örnek
// tercih edilmeyen ekrana yazdirma yolu
...
products
  ...
  });   //iPhone 9 -- An apple mobile which is nothing like apple -- 549 -- smartphones 
  ...
  products
    ...
    .forEach(({ title, price }) => console.log(`${title} -- ${price} `));  // MacBook Pro -- 1749 ....
...
console.log(sayilar);  //{smartphones: 5, laptops: 5, fragrances: 5, skincare: 5, groceries: 5, …}

//! NOT: eğer sıfırdan bir object in içini elimdeki dizideki verilerden bazılarıyla doldurmak istiyorsam, öncesinde object im boş olduğu için dot notation ile (sayilar.category) yapamam. bu yüzden square bracket kullanmalıyım (  sayilar[category] ) 
...
const neuList = customerList.map(({ id, name, email }) => ({
  id,
  name,
  email,
}));
...
console.log(elekCihaz);
console.log(kahveMak);
